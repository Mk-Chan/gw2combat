...
void incoming_strike_detection(registry_t& registry, tick_t) {
    registry
        .view<component::outgoing_damage_source, component::outgoing_strike_damage>(
            entt::exclude<component::targeting>)
        .each([&](const component::outgoing_damage_source& outgoing_damage_source,
                  const component::outgoing_strike_damage& outgoing_strike_damage) {
            auto targeting_ptr =
                registry.try_get<component::targeting>(outgoing_damage_source.source);
            if (targeting_ptr) {
